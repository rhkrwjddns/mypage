	...
	<body>
		...
		<section>
            <?php include $_SERVER['DOCUMENT_ROOT'] . "/mypage/main_img_bar.php";
				...
				//$_POST["mode"]값이 세팅되어있지 않으면 삽입(insert) 해당폼 이용하겠다.
				//$mode = "insert" 기본값 세팅됨
			...
			<div id="board_box">
				...
				<!-- //파일 첨부때 enctype="multipart/form-data"를 반드시 폼태그에 입력해야 한다. -->
				<form name="board_form" method="post" action="dmi_board.php" enctype="multipart/form-data">
						<!-- //$mode=== "modify" 수정모드, num, page를 히든으로 전송. -->
						<!-- //$mode=== "insert" 해당 문장 실행 안함. -->
                    <?php if ($mode === "modify"): ?>
